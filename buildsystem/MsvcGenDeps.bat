@echo off
echo bin\objects\%~1.msvc.dep: src\%~1.c
echo.    %0 %1 %2 %3 ^> bin\objects\%~1.msvc.dep

echo bin\objects\%~1.msvc_d.o: bin\objects\%~1.msvc.dep \
...
echo bin\objects\%~1.msvc.o: bin\objects\%~1.msvc.dep \
for /F "tokens=1,2,3,*" %%A in ('"cl /nologo /c src\%~1.c /Zs /showIncludes %~3"') do (
    if "%%A"=="Note:" if "%%B"=="including" if "%%C"=="file:" echo "%%D" \
)
echo. 
